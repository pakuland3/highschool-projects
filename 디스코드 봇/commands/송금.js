...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!user) return message.reply("**송금할 대상을 멘션해야해**") // 멘션 한 유저가 없다면 리턴
        ...
        if (!sk) return message.reply("**가입을 먼저 진행해야해 ( 범이야 돈줘 )**") // 데이터베이스에서 userid 가 메세지를 보낸사람의 id인것이 없다면 리턴
        ...
        if (!betting) return message.reply("**사용법 : 범이야 송금 @멘션 (금액)**") // 2번쨰 args에서 숫자를 못찾으면 리턴
        if (message.content.includes("-")) return message.reply("**금액엔 -가 포함되면 안돼**") // 메세지 내용에 -가 포함되면 리턴
        if (betting < 1000) return message.reply("**최소 송금 금액은 1000원이야**") // 2번쨰 args가 1000보다 작으면 리턴
